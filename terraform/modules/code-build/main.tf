locals {
  name = "${var.project}-terraform-aws"
...
module "aws_s3_bucket_artifact_name" {
  source        = "../aws-resource-naming"
  name_prefix   = "${var.project}-terraform-ci-cd-${data.aws_caller_identity.current.account_id}"
...
module "ci_codebuild_role" {
  source = "../aws-iam-role/modules/service"
